...
int get_mysql_info(char *mysql_user, char *mysql_pwd, char *mysql_db, char *mysql_ip, char *mysql_port)
{
    ...
    if (-1 == get_cfg_value(CFG_PATH, "mysql", "ip", mysql_ip))
    {
        LOG(CFG_LOG_MODULE, CFG_LOG_PROC, "mysql_ip err\n");
        return -1;
    }

    if (-1 == get_cfg_value(CFG_PATH, "mysql", "port", mysql_port))
    {
        LOG(CFG_LOG_MODULE, CFG_LOG_PROC, "mysql_port err\n");
        return -1;
    }
