...
* 
* @Functional description:显示当前用户的全部文件
...
void return_login_status(long num, int code_flag)
{
    char *out = NULL;
    char *code;
    char num_buf[128] = {0};
...
int get_user_files_count(char *user, int ret)
{
...
int get_user_filelist(char *cmd, char *user, int start, int count)
{
    ...
    LOG(MYFILES_LOG_MODULE, MYFILES_LOG_PROC, "work sql_cmd : %s \n", sql_cmd);
    ...
    while ((row = mysql_fetch_row(res_set)) != NULL)
    {
        ...
        {
        "user": "yoyo",
        "md5": "e8ea6031b779ac26c319ddf949ad9d8d",rop_zset_zrevrange
