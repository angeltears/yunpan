...
*
* @Functional description:注册事件后CGI程序
...
int user_register( char *reg_buf )
{
    ...
    char mysql_db[256] = {0};
    char mysql_ip[16] = {0};
    char mysql_port_s[5] = {0};
    ret = get_mysql_info(mysql_user, mysql_pwd,  mysql_db, mysql_ip, mysql_port_s);
    unsigned int mysql_port = atoi(mysql_port_s);
    if(ret != 0)
    {
        goto END;
    }
    LOG(REG_LOG_MODULE, REG_LOG_PROC, "mysql_user = %s, mysql_pwd = %s, mysql_db = %s, mysql_ip = %s, mysql_port = %d\n",
        mysql_user, mysql_pwd, mysql_db,mysql_ip,mysql_port);
    ...
    conn = msql_conn(mysql_user, mysql_pwd, mysql_db,mysql_ip, mysql_port);
    if (conn == NULL)
    {
    ...
    LOG(REG_LOG_PROC, REG_LOG_PROC, "%s,注册成功",user);
END:
    if(conn != NULL)
    {
